    ...
    <div class="flex max-w-md bg-white shadow-lg rounded-lg overflow-hidden">
        @if(isset($item->image) || isset($item['attributes']['image']) )
            <div class="w-1/3 bg-cover"
                 style="background-image: url('/storage/menu_images/{{$item->image??$item['attributes']['image']}}')">
            </div>
        @else
            <div class="w-1/3 bg-cover"
                 style="background-image: url('https://loremflickr.com/320/240/dish,food,steak?random={{$item->id}}')">
            </div>
        @endisset
        <div class="w-2/3 p-4 bg-background-fourth">
            ...
            <div class="flex item-center justify-between mt-3">
                ...
                <a href="/menu/{{ $item -> id }}">
                    <button class="px-3 py-2 bg-gray-800 text-white text-xs font-bold rounded"
                            aria-label="View Item in more detail">
                        View
                    </button>
                </a>
